...
import ci.miage.morpion.gnanguy.thio.Donnee;
import java.net.URL;
...
public class CheckOutPatientController implements Initializable {
    ...
    public void payBtn() {
        ...
        if (checkout_checkout.getValue() == null
            ...
            if (alert.confirmationMessage("Etes vous sure de payer ?")) {
                String sql = "INSERT INTO paiement "
                        + "(id_patient, docteur, total_jours, total_prix, date, date_paiement, date_paiement) "
                        + "VALUES(?,?,?,?,?,?,?)";
                ...
                try {
                    ...
                    String updateDonnee = "UPDATE patient SET paiement_statut = ? WHERE id_paiement= "
                            + checkout_patientID.getText();

                    prepare = connect.prepareStatement(updateDonnee);
    ...
    public void displayFields() {
        ...
        checkout_patientID.setText(String.valueOf(Donnee.temp_PatientID));
        checkout_name.setText(Donnee.temp_nom);
        checkout_doctor.setText(Donnee.temp_docteurID);
        checkout_date.setValue(LocalDate.parse(Donnee.temp_date));
        checkout_date.setDisable(true);

        image = new Image("File:" + Donnee.temp_path, 116, 132, false, true);
